...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    ...
    case ACTIONS.SET_FLASH_STATE: {
	  if (!state.get('isFlashing') && !state.get('isDownloading')) {
        throw errors.createError({
      ...
      if (!state.get('isDownloading')) {
        throw errors.createError({
          title: 'Can\'t set the flashing state when not downloading'
        })
	  }
	...
	case ACTIONS.SET_DOWNLOADING_FLAG: {
      return state
		.set('isDownloading', true)
		.set('isFlashing', false)
    }

	case ACTIONS.UNSET_DOWNLOADING_FLAG: {
		return state
		  .set('isDownloading', false)
	  }
